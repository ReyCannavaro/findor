...
const CATEGORIES_SMALL = [
  { icon: <Volume2 size={20} />, label: 'Sound System', count: 87, image: 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=400&q=80' },
  { icon: <Lightbulb size={20} />, label: 'Decoration', count: 203, image: 'https://images.unsplash.com/photo-1464366400600-7168b8af9bc3?w=400&q=80' },
  { icon: <Camera size={20} />, label: 'Documentation', count: 91, image: 'https://images.unsplash.com/photo-1492691527719-9d1e07e534b4?w=400&q=80' },
  { icon: <Music size={20} />, label: 'Entertainment', count: 64, image: 'https://images.unsplash.com/photo-1429962714451-bb934ecdc4ec?w=400&q=80' },
];

const POPULAR_VENDORS = [
  { name: 'Melody Aura Sound', category: 'Sound & Audio Specialist', location: 'Sudirman, Jakarta', price: 'Rp 15jt', rating: 4.9, reviews: 241, img: 'https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=600&q=80' },
  { name: 'Atelier Decor', category: 'Floral & Art Direction', location: 'Tangerang Selatan', price: 'Rp 45jt', rating: 5.0, reviews: 92, img: 'https://images.unsplash.com/photo-1478146059778-26028b07395a?w=600&q=80' },
  { name: 'Visual Soul Studio', category: 'Cinematic Documentation', location: 'Kemang, Jakarta', price: 'Rp 20jt', rating: 4.8, reviews: 180, img: 'https://images.unsplash.com/photo-1492691527719-9d1e07e534b4?w=600&q=80' },
  { name: 'Savory Palette', category: 'Premium Catering Group', location: 'Kelapa Gading, Jakarta', price: 'Rp 350rb', rating: 4.9, reviews: 310, img: 'https://images.unsplash.com/photo-1555244162-803834f70033?w=600&q=80' },
];
...
function HeroSection() {
  ...
  return (
    ...
    >
      <div
        key={`bg-${current}`}
        style={{ position: 'absolute', inset: 0, backgroundImage: `url('${slide.image}')`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'brightness(0.55)', animation: 'heroZoom 7s ease-out forwards' }}
      />
      ...
      <button onClick={() => goTo(current - 1)} aria-label="Sebelumnya" style={{ position: 'absolute', left: 32, top: '50%', transform: 'translateY(-50%)', zIndex: 10, width: 48, height: 48, borderRadius: '50%', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', backdropFilter: 'blur(12px)', display: 'grid', placeItems: 'center', cursor: 'pointer', color: 'white', transition: 'background 0.2s' }}>
        <ArrowLeft size={20} />
      </button>
      <button onClick={() => goTo(current + 1)} aria-label="Berikutnya" style={{ position: 'absolute', right: 32, top: '50%', transform: 'translateY(-50%)', zIndex: 10, width: 48, height: 48, borderRadius: '50%', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', backdropFilter: 'blur(12px)', display: 'grid', placeItems: 'center', cursor: 'pointer', color: 'white', transition: 'background 0.2s' }}>
        <ArrowRight size={20} />
      </button>

      <div style={{ position: 'absolute', left: '50%', bottom: 168, transform: 'translateX(-50%)', zIndex: 10, display: 'flex', alignItems: 'center', gap: 10 }}>
        {SLIDES.map((_, i) => (
          <button key={i} onClick={() => goTo(i)} aria-label={`Slide ${i + 1}`} style={{ width: i === current ? 28 : 8, height: 8, borderRadius: 100, background: i === current ? 'var(--amber)' : 'rgba(255,255,255,0.25)', border: 'none', cursor: 'pointer', padding: 0, transition: 'background 0.3s, width 0.35s cubic-bezier(0.22,1,0.36,1)' }} />
        ))}
      </div>
      ...
      <div style={{ position: 'absolute', bottom: 48, left: 0, right: 0, zIndex: 4, maxWidth: 1200, margin: '0 auto', padding: '0 48px', display: 'flex', alignItems: 'center' }}>
        {HERO_STATS.map((s, i) => (
          <div key={i} style={{ flex: 1, borderRight: i < HERO_STATS.length - 1 ? '1px solid rgba(255,255,255,0.1)' : 'none', paddingRight: i < HERO_STATS.length - 1 ? 32 : 0, marginRight: i < HERO_STATS.length - 1 ? 32 : 0, transform: statsVisible ? 'translateY(0)' : 'translateY(20px)', opacity: statsVisible ? 1 : 0, transition: `transform 0.6s ${i * 0.12}s cubic-bezier(0.22,1,0.36,1), opacity 0.6s ${i * 0.12}s ease` }}>
            <div style={{ fontFamily: 'Fraunces, serif', fontSize: 30, fontWeight: 700, color: 'var(--white)', lineHeight: 1 }}>{s.num}<span style={{ color: 'var(--amber)' }}>{s.suffix}</span></div>
  ...
  return (
    <main style={{ minHeight: '100vh' }}>
      ...
      <section style={{ padding: '80px 0', background: 'var(--white)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gridTemplateRows: 'auto auto', gap: 16 }}>
            <Link href="/search?kategori=Stage+%26+Rigging" style={{ textDecoration: 'none', gridRow: '1 / 3' }}>
              <div className="cat-card" style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', height: '100%', minHeight: 380, cursor: 'pointer', background: 'var(--forest)', display: 'flex', alignItems: 'flex-end' }}>
                <img src="https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=600&q=80" alt="Stage & Rigging" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.5s ease' }} className="cat-img" />
            ...
            <Link href="/search?kategori=Catering+Service" style={{ textDecoration: 'none', gridColumn: '2 / 4' }}>
              <div className="cat-card" style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', height: 170, cursor: 'pointer', background: 'var(--forest)', display: 'flex', alignItems: 'center' }}>
                <img src="https://images.unsplash.com/photo-1555244162-803834f70033?w=800&q=80" alt="Catering Service" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.5s ease' }} className="cat-img" />
        ...
        <style>{`.cat-card:hover .cat-img { transform: scale(1.06); } .cat-card { transition: box-shadow 0.3s, transform 0.3s; } .cat-card:hover { box-shadow: 0 16px 48px rgba(0,0,0,0.18); transform: translateY(-3px); }`}</style>
      </section>
      ...
      <section style={{ padding: '80px 0', background: 'var(--gray-50)' }}>
                ...
                <div key={v.name} className="vendor-card">
                  ...
                  <div style={{ padding: 16 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <button onClick={() => router.push(`/vendor/${slug}`)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, color: 'var(--forest)', background: 'var(--amber)', borderRadius: 100, padding: '7px 14px', border: 'none', cursor: 'pointer' }}>
        ...
        <style>{`.vendor-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; } @media (max-width: 1024px) { .vendor-grid { grid-template-columns: repeat(2, 1fr); } } @media (max-width: 480px) { .vendor-grid { grid-template-columns: 1fr; } }`}</style>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div style={{ borderRadius: 'var(--radius-lg)', overflow: 'hidden', aspectRatio: '3/4', background: 'var(--forest)', position: 'relative' }}>
                <img src="https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=600&q=80" alt="vendor" style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.7, mixBlendMode: 'luminosity' }} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ borderRadius: 'var(--radius-lg)', background: 'var(--gray-100)', padding: 16, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ borderRadius: 'var(--radius-lg)', overflow: 'hidden', aspectRatio: '1', background: 'var(--forest)', position: 'relative' }}>
                  <img src="https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=400&q=80" alt="speaker" style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.6 }} />
      ...
      <section style={{ padding: '100px 24px', background: 'var(--forest)', position: 'relative', overflow: 'hidden' }}>
        <img src="https://images.unsplash.com/photo-1429962714451-bb934ecdc4ec?w=1400&q=80" alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.55)' }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <Link href="/how-it-works" className="btn-secondary">Hubungi Consultant</Link>
          </div>
        </div>
      </section>
